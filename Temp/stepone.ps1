        ...
        Write-Host "Parametres Defender`n"
        ## windowsdefender:
...
cmd /c "bcdedit /deletevalue {current} safeboot >nul 2>&1"
        ...
        Write-Host "DDU & Redemarrage`n"

# uninstall soundblaster realtek intel amd nvidia drivers & restart (silent - no ddu windows/prompts)
Start-Process "$env:SystemRoot\Temp\ddu\Display Driver Uninstaller.exe" -ArgumentList "-silent -CleanSoundBlaster -CleanRealtek -CleanAllGpus -Restart" -Wait -WindowStyle Hidden